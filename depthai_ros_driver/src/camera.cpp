...
void Camera::setIR() {
    if(ph->getParam<bool>("i_enable_ir") && !device->getIrDrivers().empty()) {
        device->setIrLaserDotProjectorIntensity(ph->getParam<int>("i_laser_dot_brightness"));
        device->setIrFloodLightIntensity(ph->getParam<int>("i_floodlight_brightness"));
    }
}
    ...
    for(const auto& p : params) {
        if(ph->getParam<bool>("i_enable_ir") && !device->getIrDrivers().empty()) {
            if(p.get_name() == ph->getFullParamName("i_laser_dot_brightness")) {
                device->setIrLaserDotProjectorIntensity(p.get_value<int>());
            } else if(p.get_name() == ph->getFullParamName("i_floodlight_brightness")) {
                device->setIrFloodLightIntensity(p.get_value<int>());
            }
        }
    }
    for(const auto& node : daiNodes) {
